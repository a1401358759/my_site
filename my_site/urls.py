...
urlpatterns = [
    ...
    path('article/(?P<year>\d{4})/(?P<month>\d{2})/(?P<day>\d{2})/(?P<id>\d+)/', views.detail, name="detail"),  # 每篇文章
    path('article/(?P<year>\d{4})/(?P<month>\d{1,2})/', views.archive_month, name="archive_month"),  # 按月归档
    path('articleClassfi/(?P<classfi>\w+)/', views.classfiDetail, name="classfiDetail"),  # 每个分类页下面的文章
    path('articleTag/(?P<tag>\w+)/', views.tagDetail, name="tagDetail"),  # 每个标签页下面的文章
